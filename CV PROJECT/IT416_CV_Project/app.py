...
NOTEBOOK_PATH = BASE_DIR / "Image_enhancement_fixed.ipynb"
...
@app.get("/paper.pdf")
def paper_pdf():
    preferred = BASE_DIR / "paper.pdf"
    fallback = BASE_DIR / "IT416_Final_Submission.pdf"
    if preferred.exists():
        return send_from_directory(BASE_DIR, preferred.name)
    if fallback.exists():
        return send_from_directory(BASE_DIR, fallback.name)
